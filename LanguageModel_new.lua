...
function layer:__init(opt)
  ...
  self.core_img = GRU.gru(self.input_encoding_size, self.input_encoding_size, self.rnn_size, 1, dropout)
...
function layer:_createInitState_img(batch_size)
	...
	assert(batch_size ~= nil, 'batch size must be provided')
	if not sef.init_state_img then self.init_state_img = {} end
...
function layer:training()
  ...
  for k,v in pairs(self.clones_img) do v:training() end
...
function layer:evaluate()
  ...
  for k,v in pairs(self.clones_img) do v:evaluate() end
...
function layer:sample(imgs, opt)
  ...
  local state_img = self.init_state_img
  ...
  for t=1,self.seq_length+1 do
	...
	for i=1, self.num_of_local_img+1 do
		inputs = {imgFeature[i]}
		table.insert(inputs, state_img[1])
...
function layer:updateOutput(input)
  ...
  for t=1,self.seq_length+1 do
	...
	if not can_skip then
	  ...
	  self.state_img[t] = {[0] = self.init_state_img}
	  self.inputs_img[t] = {}
