...
function DataLoader:getBatch(opt)
  ...
  local getVerticalMirror = utils.getopt(opt, 'getVerticalMirror', 1)
  ...
  if getVerticalMirror == 1 then
	data.imagesMirror = torch.BypeTensor(batch_size, 3, 256, 256)
	local H = data.images:size(3)
	local W = data.images:size(4)
	for j=1,W/2 do
	  data.imagesMirror[{},{},{},{j,j}] = data.images[{},{},{},{W-j+1,W-j+1}]
	end
  end
  ...
  data.backlabels = torch.LongTensor(data.labels:size(1),data.labels:size(2)):zero()
  for loop1=1,data.labels:size(2) do
	local loop2=0
	for loop2=1,data.labels:size(1) do
		if data.labels(loop2,loop1)==0 then
			break
		end
	end
	loop2=loop2-1
	for loop3=1,loop2 do
		data.backlabels(loop3,loop1)=data.labels(loop2-loop3+1,loop1)
	end
  end
