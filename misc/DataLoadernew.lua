...
function DataLoader:getBatch(opt)
  ...
  data.images_mirro = net_utils.HorizonalMap(img_batch_raw, 1)
  data.labels = label_batch:transpose(1,2):contiguous() -- note: make label sequences go down as columns
  data.bounds = {it_pos_now = self.iterators[split], it_max = #split_ix, wrapped = wrapped}
  data.infos = infos


  assert(data.labels:size(1)==self.seq_length,'affirm structure of data.labels is correct')
  data.labels_back = torch.LongTensor(data.labels:size(1),data.labels:size(2)):zero()
  for loop1=1,data.labels:size(2) do
	local top = data.labels:size(1)+1
	for loop2 = 1,data.labels:size(1) do
		if torch.sum(data.labels:sub(loop2,loop2,loop1,loop1)) == 0 then
			top = loop2
			break
		end
	end
	for loop3=1,top-1 do
		data.labels_back[loop3][loop1]=data.labels[top-loop3][loop1]
	end
  end

  data.labels_special = torch.LongTensor(data.labels:size(1),data.labels:size(2)):zero()
  for loop1 =1,batch_size do
	local goal = 0
	local max_len = 0
	for loop2 =1,seq_per_img do
		local top = data.labels:size(1)+1
		local col = (loop1-1)*seq_per_img + loop2
		for loop3 = 1,data.labels:size(1) do
			if torch.sum(data.labels:sub(loop3,loop3,col,col)) == 0 then
				top = loop3
				break
			end
		end
	...
	for loop2 =1,seq_per_img do
		local col = (loop1-1)*seq_per_img + loop2
		data.labels_special:sub(1,self.seq_length,col,col):copy(data.labels:sub(1,self.seq_length,goal,goal))
	end
  end
