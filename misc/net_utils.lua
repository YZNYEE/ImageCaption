...
function net_utils:MirrorMap(seq, vocab_size)
	local D,N = seq:size(1),seq:size(2)
	local out = torch.LongTensor(D,N):zero()
	for i=1,N do
		local pos=D+1
		for j=1,D do
			if seq[{j,i}] == vocab_size+1 then pos = j break end
		end
		-- assert( pos ~= 1,'MirrorMap is wrong')
		for j=1,pos-1 do
			out[{j,i}] = seq[{pos-j,i}]
		end
		if pos ~= D+1 then
			out[{pos, i}] = vocab_size+1
		end
	end
	return out
end

function net_utils.HorizonalMap(image,flag)
  local batch_size = image:size(1)
  local channels = image:size(2)
  local width = image:size(3)
  local length = image:size(4)
  assert(channels == 3,'img is error')
  assert(width == length,'don not match')
  local dimension
  if flag == 0 then dimension = 3
  else dimension = 4
  end
  local image_mirro = torch.ByteTensor(image:size())
  for i=1,width do
    if dimension == 3 then
      image_mirro:sub(1,batch_size,1,channels,i,i,1,length):copy(image:sub(1,batch_size,1,channels,width-i+1,width-i+1,1,length))
    else
	  image_mirro:sub(1,batch_size,1,channels,1,width,i,i):copy(image:sub(1,batch_size,1,channels,1,width,length-i+1,length-i+1))
	end
  end
  return image_mirro
end
...
function net_utils.decode_sequence_new(ix_to_word, seq, seqLogprobs)
  local D,N = seq:size(1), seq:size(2)
  local out = {}
  local t = torch.LongTensor(N):zero()
  local seqprobsMean = torch.FloatTensor(N):zero()
  local seqprobs = torch.exp(seqLogprobs)
  for i=1,N do
    local txt = ''
	local top=D+1
	local sum=0
    for j=1,D do
      local ix = seq[{j,i}]
      local word = ix_to_word[tostring(ix)]
      if not word then top=j break end -- END token, likely. Or null token
      sum=sum+seqprobs[{j,i}]
	  if j >= 2 then txt = txt .. ' ' end
      txt = txt .. word
    end
	t[i]=top
	-- assert(top~=1,'operate error')
	seqprobsMean[i] = sum/(top-1)
    table.insert(out, txt)
  end
  return out,t,seqprobsMean
end

function net_utils.decode_sequence_new_informationentropy(ix_to_word, seq, seqLogprobs)
  local D,N = seq:size(1), seq:size(2)
  local out = {}
  local t = torch.LongTensor(N):zero()
  local seqprobsMean = torch.FloatTensor(N):zero()
  ...
  for i=1,N do
    ...
    for j=1,D do
      ...
      local word = ix_to_word[tostring(ix)]
      if not word then top=j break end -- END token, likely. Or null token
      sum=sum-seqprobs[{j,i}]*seqLogprobs[{j,i}]
	  if j >= 2 then txt = txt .. ' ' end
      txt = txt .. word
    end
	t[i]=top
...
function net_utils.getInformationEntropy(seqLogprobs)
  local D,N = seqLogprobs:size(1), seqLogprobs:size(2)
  local seqprobs = torch.exp(seqLogprobs)
  local seqprobsMean = torch.FloatTensor(N):zero()
  for i=1,N do
    sum = 0
...
function net_utils.language_eval_forFlickr8k(predictions, id, split)
  -- this is gross, but we have to call coco python code.
  -- Not my favorite kind of thing, but here we go
  local out_struct = {val_predictions = predictions}
  utils.write_json('coco-caption/val' .. id .. '.json', out_struct) -- serialize to json (ew, so gross)
  os.execute('./misc/call_python_caption_eval_forFlickr8k.sh val' .. id .. '.json ' .. split) -- i'm dying over here
  local result_struct = utils.read_json('coco-caption/val' .. id .. '.json_flickr8k_out.json') -- god forgive me
  return result_struct
end

function net_utils.language_eval_forFlickr30k(predictions, id)
  -- this is gross, but we have to call coco python code.
  -- Not my favorite kind of thing, but here we go
  local out_struct = {val_predictions = predictions}
  utils.write_json('coco-caption/val' .. id .. '.json', out_struct) -- serialize to json (ew, so gross)
  os.execute('./misc/call_python_caption_eval_forFlickr30k.sh val' .. id .. '.json') -- i'm dying over here
  local result_struct = utils.read_json('coco-caption/val' .. id .. '.json_flickr30k_out.json') -- god forgive me
  return result_struct
end

function net_utils.language_eval_forCoco(predictions, id)
  -- this is gross, but we have to call coco python code.
  -- Not my favorite kind of thing, but here we go
  local out_struct = {val_predictions = predictions}
  utils.write_json('coco-caption/val' .. id .. '.json', out_struct) -- serialize to json (ew, so gross)
  os.execute('./misc/call_python_caption_eval_forCoco.sh val' .. id .. '.json') -- i'm dying over here
  local result_struct = utils.read_json('coco-caption/val' .. id .. '.json_coco_out.json') -- god forgive me
  return result_struct
end
