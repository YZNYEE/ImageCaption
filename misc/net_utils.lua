...
function net_utils.build_cnn(cnn, opt)
  --utils.getopt(a,b,c) ÆäÖÐcÎªÄ¬ÈÏ²ÎÊý
  ...
  --nn.Sequential()ÊÇÈÝÆ÷Àà£¬¶ÓÁÐÐÍÈÝÆ÷
  ...
  for i = 1, layer_num do
    ...
    if i == 1 then
      ...
      --½«BGRÐÎÊ½µÄ²ÎÊýÐÎÊ½×ª»»ÎªRGB¸ñÊ½µÄ²ÎÊý
	  --Clone²ÎÊý
  ...
  --ÕâÊ±ÒÑ¾­µÃµ½µÄÊÇcnnµÄ×îºóÒ»²ã
  --Èç¹ûÏëÒªÌáÈ¡Ç°Ãæ¼¸²ã¿ÉÒÔÔÚÕâÖØÐ´
  --ÔÚ×îºóÒ»²ãÌí¼Óµ½encoding_sizeÎ¬¶ÈµÄ×ª»»
  cnn_part:add(nn.Linear(4096,encoding_size))
  --Ìí¼Ó·ÇÏßÐÔ²ã
  --ÕâÀïbackendÎªcunn×Ö·û´®
  cnn_part:add(backend.ReLU(true))
  return cnn_part
end

--ÌáÈ¡batchsize³¤¶ÈµÄimages²¢ÇÒ½øÐÐÔ¤´¦Àí
-- takes a batch of images and preprocesses them
-- VGG-16 network is hardcoded, as is 224 as size to forward
-- VGG-16 ÍøÂçÊÇÐ´ËÀµÄÍøÂç£¬224´óÐ¡ÊÇÍøÂç³õÊ¼²ã¹Ì¶¨µÄ´óÐ¡
function net_utils.prepro(imgs, data_augment, on_gpu)
  ...
  if h > cnn_input_size or w > cnn_input_size then
    ...
    if data_augment then
	  --ÕâÓëÏÂÃæ²»Í¬Ö®´¦ÊÇËæ»ú´ÓÍ¼Æ¬ÖÐÌáÈ¡224´óÐ¡µÄÇøÓò
...
function layer:updateOutput(input)
  ...
  for k=1,input:size(1) do
	--ÕâÀïµÄKÒ²¿ÉÒÔ¿´×÷ÊÇÖ¸¶¨µÄÊÇµÚ¼¸ºÅÊý¾Ý
    local j = (k-1)*self.n+1
	--ÖµµÃ×¢ÒâµÄÊÇÕâÀïµÄÊý¾ÝÊÇ³É¿éµÄ£¬¼´jÓëj+selfÖ®¼ä¶¼ÊÇÏàÍ¬µÄÊý¾Ý£
...
function layer:updateGradInput(input, gradOutput)
  ...
  for k=1,input:size(1) do
    local j = (k-1)*self.n+1
	--¶ÔÃ¿Ò»ÁÐ½øÐÐÒ»¸öÇóºÍ²Ù×÷£¬¼´ÏàÍ¬Êý¾Ý¶ÔÓ¦µÄÌÝ¶ÈÇóºÍ
...
function net_utils.listModules(net)
  ...
  --listModules½«netÖÐµÄModules×ª»»³ÉLIST
